...
if (!class_exists('foodbakery_publisher_restaurant_actions')) {
	...
	public function title_description($html = '') {
	    ...
	    $html .= '<div class="foodbakery-dev-appended-cats">' . $this->restaurant_categories($restaurant_type_id, $get_restaurant_id) . '</div>';
            $html = apply_filters('foodbakery_restaurant_fields_frontend', $html, $get_restaurant_id);
	    ...
	    $form_html .= '
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div class="field-holder">
					...
					<input class="menu-item-title" data-js_id="_menu_cat_title" id="menu_item_title_' . $menu_item_counter . '"' . $title_name_value . ' type="text" placeholder="' . esc_html__('Menu Category Title', 'foodbakery') . '">	
				</div>
			</div>';
            $form_html  .= apply_filters('foodbakery_menu_category_fields_frontend','', $menu_item_counter, $get_menu_cat_vals, $doin_action);
            $form_html  .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div class="field-holder">
					<label>' . esc_html__('Description', 'foodbakery') . '</label>
					<textarea class="menu-item-desc" data-js_id="_menu_cat_desc" id="menu_item_desc_' . $menu_item_counter . '"' . $desc_name . ' placeholder="' . esc_html__('Category Description', 'foodbakery') . '">' . $desc_val . '</textarea>
				</div>
			</div>';
	    $form_html .= '
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
	...
	public function foodbakery_restaurant_menu_cat_item($menu_item_counter = '', $menu_cat_vals = array()) {
		...
		'menu_cat_desc' => $menu_cat_desc,
            ...
            $get_menu_cat_vals  = apply_filters('foodbakery_menu_cat_array_values',$get_menu_cat_vals, $menu_cat_vals);
	...
	public function group_restaurant_menu_cats($restaurant_id) {
		...
		foreach ($restaurant_menu_cat_titles as $cat_title) {
			...
			'menu_cat_desc' => $cat_desc,
                    ...
                    $get_menu_cat_vals  = apply_filters('foodbakery_menu_cat_array',$get_menu_cat_vals, $restaurant_id, $cat_counter);
	...
	public function restaurant_opening_hours($type_id = '', $foodbakery_id = '') {
		...
		$get_restaurant_id = foodbakery_get_input('restaurant_id', 0);
                ...
                $get_opening_hours = $foodbakery_opening_hours_element->foodbakery_opening_hours_array($foodbakery_id);
		...
		if (is_array($week_days) && sizeof($week_days) > 0) {
			...
			if (is_array($time_list) && sizeof($time_list) > 0) {
			    ...
			    foreach ($time_list as $time_key => $time_val) {
                                $time_key   = apply_filters('foodbakery_time_format', $time_key);
				$time_from_html .= '<option value="' . $time_key . '"' . ($opening_time == $time_key ? ' selected="selected"' : '') . '>' . apply_filters('foodbakery_time_format', $time_val) . '</option>' . "\n";
				$time_to_html .= '<option value="' . $time_key . '"' . ($closing_time == $time_key ? ' selected="selected"' : '') . '>' . apply_filters('foodbakery_time_format', $time_val) . '</option>' . "\n";
			    }
			}
							...
							<div id="open-close-con-' . $day_key . '-' . $restaurant_add_counter . '" class="open-close-time' . (isset($day_status) && $day_status == 'on' ? ' opening-time' : '') . '">
								...
								<div class="time-sec">
									...
									<select class="chosen-select " name="foodbakery_opening_hour[' . $day_key . '][closing_time]">
										' . $time_to_html . '
									</select>'.apply_filters('wp_multi_opening_hour_input_fields_frontend', '',$day_key, $get_opening_hours, $time_list).'
	...
	public function restaurant_meta_save() {
		...
		if (isset($_POST['menu_cat_title'])) {
		    ...
		    update_post_meta($restaurant_id, 'menu_cat_descs', $restaurant_menu_cat_descs);
                    ...
                    do_action('foodbakery_save_menu_categories', $restaurant_id);
		}
		...
		$foodbakery_restaurant_menu_item_price = foodbakery_get_input('menu_item_price', '', 'ARRAY');
                $foodbakery_restaurant_menu_item_calories = foodbakery_get_input('menu_item_calories', '', 'ARRAY');
		$foodbakery_restaurant_menu_item_stock = foodbakery_get_input('menu_item_stock', '', 'ARRAY');
		...
		if (isset($_POST['menu_item_title']) && is_array($foodbakery_restaurant_menu_item_title) && sizeof($foodbakery_restaurant_menu_item_title) > 0) {
		    ...
		    foreach ($foodbakery_restaurant_menu_item_title as $key => $menu_item) {
                        $menu_rand_id = rand(10000000, 99999999);
			...
			if (count($menu_item) > 0 && $menu_item != '' && $menu_item_action != 'add') {
				...
				'menu_item_price' => isset($foodbakery_restaurant_menu_item_price[$key]) ? $foodbakery_restaurant_menu_item_price[$key] : '',
                                'menu_item_calories' => isset($foodbakery_restaurant_menu_item_calories[$key]) ? $foodbakery_restaurant_menu_item_calories[$key] : '',
				'menu_item_stock' => isset($foodbakery_restaurant_menu_item_stock[$key]) ? $foodbakery_restaurant_menu_item_stock[$key] : '',
				'menu_item_identity' => $menu_rand_id,
