...
/*
 * Square Gateway Listener
 */
if (isset($_POST['square_payment_id'])) {
    // Square payment ID is present, proceed with processing
    $payment_id = $_POST['square_payment_id'];
    // Use Square API to verify and get payment details
    // Assuming you have a function to get payment details from Square API

    $payment_details = get_square_payment_details($payment_id);

    if ($payment_details['status'] == 'COMPLETED') {
        $foodbakery_id = $payment_details['item_number'];
        $transaction_array = array();

        $transaction_array['foodbakery_trans_id'] = esc_attr($payment_details['transaction_id']);
        $transaction_array['foodbakery_post_id'] = esc_attr($foodbakery_id);
        $transaction_array['foodbakery_transaction_status'] = 'approved';
        $transaction_array['foodbakery_full_address'] = esc_attr($payment_details['address']);
        $transaction_array['foodbakery_transaction_amount'] = esc_attr($payment_details['amount']);
        $transaction_array['foodbakery_trans_currency'] = esc_attr($payment_details['currency']);
        $transaction_array['foodbakery_summary_email'] = esc_attr($payment_details['payer_email']);
        $transaction_array['foodbakery_first_name'] = esc_attr($payment_details['first_name']);
        $transaction_array['foodbakery_last_name'] = esc_attr($payment_details['last_name']);

        $foodbakery_trans_id = isset($payment_details['custom']) ? $payment_details['custom'] : '';

        foodbakery_update_transaction($transaction_array, $foodbakery_trans_id);
        foodbakery_update_post($foodbakery_id, $foodbakery_trans_id, $payment_details['status']);
    }
}

function get_square_payment_details($payment_id) {
    // Your code to interact with Square API and retrieve payment details
    // Return an array with payment details similar to PayPal and Authorize.net
}
